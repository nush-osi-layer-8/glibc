...
__clock_gettime64 (clockid_t clock_id, struct __timespec64 *tp)
{
  ...
  switch (clock_id)
    ...
    case CLOCK_REALTIME:
      {
	retval = gettimeofday (&tv_32, NULL);
	if (retval == 0)
	  valid_timeval_to_timespec64 (&tv_32, tp);
      }
      break;
#endif
    ...
    default:
	  ...
	  == CLOCK_THREAD_CPUTIME_ID)
        {
	  retval = hp_timing_gettime (clock_id, &ts_32);
          if (retval == 0)
            valid_timespec_to_timespec64 (&ts_32, tp);
        }
      else
#endif
	__set_errno (EINVAL);
    ...
    case CLOCK_PROCESS_CPUTIME_ID:
      retval = hp_timing_gettime (clock_id, &ts_32);
      if (retval == 0)
        valid_timeval_to_timespec64 (&ts_32, tp);
      break;
#endif
    }
